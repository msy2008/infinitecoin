Infinitecoin Core version 0.13.2 is now available from:

  <https://download.infinitecoin.com/infinitecoin-0.13.2.0/>
...
updates will be released anymore. Without security updates, using a infinitecoin
...
ECDSA signatures inside Infinitecoin transactions now use validation using
...
Previous versions of Infinitecoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Infinitecoin Core was to raise the default minimum
relay fee.

Infinitecoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Infinitecoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running infinitecoind without having to do any manual configuration.
...
Infinitecoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Infinitecoin Core will relay transactions with
...
entirely in Infinitecoin Core 0.13, and it is at this time undecided whether the
...
Infinitecoin Core automatically creates a hidden service to listen on, without
manual configuration. Infinitecoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Infinitecoin Core is listening, and
...
Infinitecoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Infinitecoin
Core to use floating fees. By default, Infinitecoin Core will use floating
fees.
...
(default: `0.0002` IFC/kB).

At all times, Infinitecoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) IFC.
Furthermore, Infinitecoin Core will never create transactions paying less than
...
`infinitecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29322 to a RPC server bound on localhost on port 19322 do:

    stunnel -d 29322 -r 127.0.0.1:19322 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /infinitecoinrpc>
        ProxyPass http://127.0.0.1:9322/
        ProxyPassReverse http://127.0.0.1:9322/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass infinitecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< infinitecoinrpc:<password>
    </Location>
...
Restarting infinitecoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in infinitecoin.conf
...
infinitecoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/infinitecoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Infinitecoin Core code base has
...
- `infinitecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `infinitecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Infinitecoin scripting system that in combination with
...
Infinitecoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Infinitecoin Core.
...
The two phases are distinct in the Infinitecoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Infinitecoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Infinitecoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Infinitecoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of infinitecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of infinitecoins being spent is exactly
...
- **Increased security for multisig:** Infinitecoin addresses (both P2PKH addresses
  ...
  believed to be provided by Infinitecoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Infinitecoin's current ledger (called the UTXO set),
  ...
  note that Infinitecoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Infinitecoin users to individually opt-in to almost any change in the Infinitecoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Infinitecoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Infinitecoin
client to Infinitecoin Core 0.13.2 to signal for segwit support. When a
...
Since Infinitecoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `infinitecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `infinitecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
